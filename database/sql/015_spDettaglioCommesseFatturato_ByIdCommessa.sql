/*
    Revisione radicale di produzione.spDettaglioCommesseFatturato
    - elimina la dipendenza da CDG.CDG_IncrocioContabilitaIntranet
    - usa l'id commessa (dbo.Commesse.ID) per filtrare i movimenti
    - mantiene invariato il contratto dei 3 result-set

    Assunzioni concordate:
    - CdgFattureAttive: filtro per commessa tramite id_cont_commessa
    - CdgFatturePassive: filtro per commessa tramite idcommessa
    - Causale: CB_CodiceCausale
    - Sottoconto:
        * Attive -> IDSottoconto
        * Passive -> Sottoconto
*/
...
/* ============================================================
   Indici di supporto al filtro per IdCommessa
   ============================================================ */
...
(
    ...
    WHERE object_id = OBJECT_ID(N'CDG.CdgFattureAttive')
      AND name = N'IX_CdgFattureAttive_IdContCommessa'
)
BEGIN
    CREATE NONCLUSTERED INDEX [IX_CdgFattureAttive_IdContCommessa]
    ON [CDG].[CdgFattureAttive] ([id_cont_commessa])
    INCLUDE
    (
        [data],
        [numero],
    ...
    WHERE [id_cont_commessa] IS NOT NULL;
END
GO
...
(
    ...
    WHERE object_id = OBJECT_ID(N'CDG.CdgFatturePassive')
      AND name = N'IX_CdgFatturePassive_IdCommessa'
)
BEGIN
    CREATE NONCLUSTERED INDEX [IX_CdgFatturePassive_IdCommessa]
    ON [CDG].[CdgFatturePassive] ([idcommessa])
    INCLUDE
    (
        [Data Documento],
        [NumeroDocumento],
...
BEGIN
    ...
    SELECT
        ...
        CAST(LEFT(LTRIM(RTRIM(ISNULL(CAST(a.[CB_CodiceCausale] AS NVARCHAR(256)), N''))), 256) AS NVARCHAR(256)) AS Causale,
        CAST(LEFT(LTRIM(RTRIM(ISNULL(CAST(a.[IDSottoconto] AS NVARCHAR(256)), N''))), 256) AS NVARCHAR(256)) AS Sottoconto,
    ...
    WHERE a.[id_cont_commessa] = @IdCommessa
    OPTION (RECOMPILE);
    ...
    SELECT
        ...
        CAST(LEFT(LTRIM(RTRIM(ISNULL(CAST(p.[CB_CodiceCausale] AS NVARCHAR(256)), N''))), 256) AS NVARCHAR(256)) AS Causale,
        CAST(LEFT(LTRIM(RTRIM(ISNULL(CAST(p.[Sottoconto] AS NVARCHAR(256)), N''))), 256) AS NVARCHAR(256)) AS Sottoconto,
